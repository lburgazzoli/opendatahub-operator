...
func TestRegistrationAndDeepCopy(t *testing.T) {
	...
	source.Spec.Components.Dashboard.ManagementState = operatorv1.Managed

	copy := source.DeepCopy()
	copy.Labels["source"] = "changed"
	copy.Spec.Components.Dashboard.ManagementState = operatorv1.Removed
	g.Expect(source.Labels["source"]).To(Equal("v3"))
	g.Expect(source.Spec.Components.Dashboard.ManagementState).To(Equal(operatorv1.Managed))
