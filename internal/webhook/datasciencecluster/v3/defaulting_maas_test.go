...
func TestMaaSProvenanceAdmission(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "canonical removal", change: func(dsc *dscv3.DataScienceCluster) {
			dsc.Spec.Components.AIGateway.ModelsAsAService.ManagementState = operatorv1.Removed
		}},
		{name: "canonical omission", change: func(dsc *dscv3.DataScienceCluster) {
			dsc.Spec.Components.AIGateway.ModelsAsAService.ManagementState = ""
		}},
		{name: "KServe parent", change: func(dsc *dscv3.DataScienceCluster) { dsc.Spec.Components.Kserve.ManagementState = operatorv1.Removed }},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			old.Spec.Components.AIGateway.ModelsAsAService.ManagementState = operatorv1.Managed
			old.Annotations = map[string]string{"unrelated": "kept"}
			old.PreserveMaaSV2State(operatorv1.Managed)
...
func TestMaaSProvenanceCreateAndErrors(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			current.PreserveMaaSV2State(operatorv1.Managed)
			ctx := admission.NewContextWithRequest(t.Context(), admission.Request{AdmissionRequest: admissionv1.AdmissionRequest{
