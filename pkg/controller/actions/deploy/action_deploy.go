...
func (a *Action) Execute(ctx context.Context, rr *odhTypes.ReconciliationRequest) error {
	for i := range rr.Resources {
		...
		old, err := a.lookup(ctx, rr.Client, obj)
		if err != nil {
		...
		case gvk.OdhDashboardConfig:
			// The OdhDashboardConfig should only be created once, or create
			// if no existing, but should not be updated
			//
			// TODO: this seems weird and we should update all the resources
			//       in the manifest. The OdhDashboardConfig should probably
			//       better defined in the manifests, i.e. the CRD should
			//       have its own defaults and the manifest should be pretty
			//       empty.
		...
		default:
			...
			switch a.deployMode {
			case ModePatch:
				err = a.patch(ctx, rr.Client, obj, old)
			case ModeSSA:
				err = a.apply(ctx, rr.Client, obj, old)
			default:
				err = fmt.Errorf("unsupported deploy mode %d", a.deployMode)
			}
...
func (a *Action) patch(ctx context.Context, c *odhClient.Client, obj unstructured.Unstructured, old *unstructured.Unstructured) error {
	...
	case gvk.Deployment:
		...
		if old == nil || old.GetAnnotations()[annotations.ManagedByODHOperator] == "true" {
			break
		}
